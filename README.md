...
- **Operational DB** — SQL Server (`customers`, `accounts`) loaded by `scripts/load_operational_db.py`
...
| `docs/STAGE1_land_in_fabric.md` | How to land the sources into Fabric |
| `GETTING_STARTED.md` | Set up SQL Server + the loader with uv |
| `data/` | The (synthetic) source data + `SOURCES.md` |
| `scripts/` | SQL Server loader + merchant REST API |
...
python scripts/load_operational_db.py --full     # load SQL Server source
...
## Tech
Microsoft Fabric (Lakehouse, Copy job, dbt job, pipelines) · dbt · SQL Server (SSMS) ·
